...
digest=$(cat "$source_root/host.mjs" "$source_root/extension-host.mjs" "$source_root/environment.mjs" "$source_root/package-lock.json" "$source_root/node-runtimes.json" | $hash_command | cut -d ' ' -f 1)
