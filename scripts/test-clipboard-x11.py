...
with tempfile.TemporaryDirectory(prefix='workbench-clipboard-test-') as tmp:
    ...
    try:
        ...
        subprocess.run(['cargo', 'test', '-p', 'workbench-runtime', 'native_x11_test', '--', '--ignored', '--nocapture'], env=env, check=True, timeout=600)
    finally:
        os.close(read_fd)
        server.terminate()
        try:
